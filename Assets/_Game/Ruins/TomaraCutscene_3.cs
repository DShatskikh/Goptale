...
public sealed class TomaraCutscene_3 : MonoBehaviour
{
    [SerializeField]
    ...
    private IEnumerator AwaitTomaraCutscene_4()
    {
        ...
        if (Stats.Instance.LV == 2) // Всех троих
        {
            ...
            yield return DialogueWindow.StartDialogue(new [] {
                "\\T1Глазам не верю!%*Расскидал всех поциков.",
                "\\T1Это конечно круто, но если ты продолжишь решать все проблемы кулаками...",
                "\\T1То весь район будет тебя боятся и считать быдлом.",
                "\\T1И когда тебе понадобится помощь, то никто не придёт.",
                "\\T1Идём.",
            });
        }
        else if (Stats.Instance.EXP > 0) // Не всех
        {
            ...
            Fedya.Instance.enabled = false;
            yield return DialogueWindow.StartDialogue(new [] {
                "\\T1Похоже ты не пропадешь.",
                "\\T2Но постарайся больше никого не бить.",
                "\\T2А то весь район будет считать тебя быдлом...",
                "\\T2...и никто не поможет в трудную минуту.",
                "\\T1Идём.",
            });
        }
        else // Пацифист
        {
            ...
            yield return DialogueWindow.StartDialogue(new [] {
                ...
                "\\T1Ты молодец и всё сделал правльно.",
                "\\T1Иначе бы тебя начали считать быдлом, а не четким поциком.",
                "\\T1Однажды тебе может понадобится помощь и тогда весь район...",
                "\\T1...придет чтобы тебе помочь.",
                "\\T1Идём.",
            });
        }
