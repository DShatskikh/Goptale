...
public sealed class Tomara : Enemy
{
    [SerializeField]
    ...
    private IEnumerator Start()
    {
        ...
        TomaraCutscene_9.IsRunning = true;
        ...
        {
            new[] {"Томара - АТК 88 ЗЩТ 88*Знает, что для вас лучше."},
            new[] {"Вы не можете придумать хоть какую-либо тему для разговора."}
        };

        if (_mercy < 8)
        {
            BattleManager.MainText = "Томара преграждает дорогу!";
        }
        else if (_mercy < 16)
        {
            BattleManager.MainText = "Томара готовит магическую атаку.";
        }
        else
        {
        ...
        while (true)
        {
            ...
            BattleManager.MainText = "Томара смотрит сквозь вас.";
            ...
            if (BattleManager.Instance.SelectMainButtonIndex == 1)
            {
                ...
                else if (BattleManager.Instance.SelectBazarIndex == 2)
                {
                    ...
                    else if (_speakerCount == 2)
                    {
                        ...
                        BattleManager.MainText = "Томара действует отстранённо.";
                    }
            ...
            else if (BattleManager.Instance.SelectMainButtonIndex == 3)
            {
                ...
                _textBubble.SetText(_mercy switch
                {
                    ...
                    4 => "Что ты делаешь?",
                    5 => "Сражайся или беги!",
                    6 => "Что ты хочешь этим сказать?",
                    7 => "Сражайся или уходи!",
                    8 => "Прекрати это.",
                    9 => "Прекрати на меня так смотреть.",
                    10 => "Уйди!",
                    11 => "...",
                    12 => "...\n...",
                    13 => "знаю, ты хочешь домой, но...",
                    14 => "Но пожалуйстра, иди наверх...",
                    ...
                    20 => ".....",
                    21 => "ха-ха..",
                    22 => "Жалко, не правда ли? Я не могу спасти даже одного поцанчика.",
                    23 => "...",
                    24 => "Нет я понимаю",
                    _ => "..."
                });
                ...
                if (_mercy == 24)
                {
                    ...
                    _textBubble = Instantiate(Resources.Load<TextBubble>("TextBubbleMini"));
                    _textBubble.transform.position = transform.position + new Vector3(1.3f, y, 0);
                    _textBubble.SetText("РУИНИНО становится слишком мал как ты только начинаешь осваивать его.");
                    yield return new WaitUntil(() => _textBubble == null);
                    
                    _textBubble = Instantiate(Resources.Load<TextBubble>("TextBubbleMini"));
                    _textBubble.transform.position = transform.position + new Vector3(1.3f, y, 0);
                    _textBubble.SetText("Будет неправильно расти вместе подобному этому.");
                    yield return new WaitUntil(() => _textBubble == null);
                    
                    _textBubble = Instantiate(Resources.Load<TextBubble>("TextBubbleMini"));
                    _textBubble.transform.position = transform.position + new Vector3(1.3f, y, 0);
                    _textBubble.SetText("Моё ожидание.*Моё одиночество.*Мои страхи.");
                    yield return new WaitUntil(() => _textBubble == null);
                    
                    _textBubble = Instantiate(Resources.Load<TextBubble>("TextBubbleMini"));
                    _textBubble.transform.position = transform.position + new Vector3(1.3f, y, 0);
                    _textBubble.SetText("Ради тебя поцанчик.*Я отодвину их назад.");
                    yield return new WaitUntil(() => _textBubble == null);
            ...
            // Сделать большое поле
            BattleManager.Instance.IsEnemyTurn = false;
            TomaraCutscene_9.IsRunning = false;
